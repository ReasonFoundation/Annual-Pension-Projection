...
latest_returns_data <- read.csv(ReturnsDataFile)
...
#Final projection with 2023 return input
return_2023 <- 0   #input the expected 2023 return here (currently default to 0%)

ppd_project_final <- ppd_benchmark %>% 
  left_join(latest_returns_data) %>% 
  group_by(plan_name) %>% 
  mutate(return = return_f(return, fy, latest_return, predict_return, input_return = return_2023),
